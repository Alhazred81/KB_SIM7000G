...
String signalHint() {
  ...
  if(csq == 99) return "Jelerosseg ismeretlen (CSQ=99).";
  if(csq <= 0) return "Nincs hasznalhato GSM jel (CSQ=" + String(csq) + ").";
  if(csq < 7) return "Gyenge GSM jel (CSQ=" + String(csq) + "/31).";
...
// ════════════════════════════════════════════════════════════
//  Adatkapcsolat (PDP context) kezelese - AT+CNACT / AT+SNPING4
// ════════════════════════════════════════════════════════════
struct DataConnState {
  bool    active       = false;  // van-e aktiv adatkapcsolat (PDP context)
  String  ip           = "";     // a kapott IP cim, ha aktiv
  String  lastError    = "";
  bool    inProgress   = false;  // valtas (be/ki) folyamatban
  // Ping teszt allapota
  bool    pingInProgress = false;
  String  pingTarget     = "";
  String  pingResult     = ""; // "" = meg nincs eredmeny, kulonben szoveges osszefoglalo
  bool    pingOk          = false;
};
extern DataConnState gData;

// ─── Adatkapcsolat bekapcsolasa (AT+CNACT=1,1) ──────────────
// Nem valtoztat azon, hogy a GSM regisztracio maga fennall-e (az a
// hangkapcsolat/SMS-hez is kell) - ez kulon a PDP context (internet-
// eleresu adatkapcsolat) be/kikapcsolasarol szol.
...
// FONTOS: nem a +CMTI URC-t figyeljuk folyamatosan (az azt jelentene,
// hogy a soros port olvasasa versenyezne minden mas blokkolo AT-parancs
// valaszaval - hivas, SMS-kuldes, modem-init). Ehelyett periodikusan,
// pl. 30 masodpercenkent, amikor BIZTOSAN senki mas nem hasznalja a
// modemet, lekerdezzuk az osszes olvasatlan uzenetet AT+CMGL-lel, es
// utana toroljuk oket a SIM-rol (hogy legkozelebb ne jelenjenek meg
// ujra "unread"-kent).

// Fix tombmeret a HARD_MAX-hoz igazodik (ezt sose lepjuk tul), de a
// tenylegesen hasznalt kapacitas futasidoben allithato (gSmsInboxLimit),
// EEPROM-ban tarolva - igy a felhasznalo 1 es SMS_INBOX_HARD_MAX kozott
// barmilyen erteket beallithat ujraforditas nelkul.
struct ReceivedSms {
  String sender;
  String timestamp;     // a SIM/halozat altal adott formatumban, pl. 24/01/15,10:30:00+04
  String ourTimestamp;  // a mi sajat, megbizhato idobelyegunk (NTP vagy modem-CCLK alapjan)
  String text;
  unsigned long receivedAt; // millis(), amikor MI eszleltuk
};
ReceivedSms gSmsInbox[SMS_INBOX_HARD_MAX];
int gSmsInboxCount = 0;               // hany ervenyes bejegyzes van (<= gSmsInboxLimit)
int gSmsInboxHead  = 0;               // kovetkezo irasi pozicio (korkoros, gSmsInboxLimit szerint)
int gSmsInboxLimit = SMS_INBOX_DEFAULT_LIMIT; // futasidoben allithato tenyleges kapacitas
unsigned long gSmsTotalReceived = 0;  // osszesen, a bootolas ota
...
// ════════════════════════════════════════════════════════════
struct LedConfig {
  uint8_t mode      = DEFAULT_LED_MODE;   // LED_MODE_V10/V11/CUSTOM/AT_NETLIGHT
  uint8_t customPin = DEFAULT_CUSTOM_PIN;
  bool    triggerOn = false;              // toggle allapot (csak manualOverride alatt szamit)
  // FONTOS: a trigger "be/ki" allapota onmagaban NEM eleg annak eldontesehez,
  // hogy a LED-et az automatikus villogo-minta vezerelje-e. Korabban, ha
  // triggerOn=false volt, az automatikus villogas AZONNAL visszavette az
  // iranyitast minden loop() korben, gyakorlatilag lathatatlanna teve a
  // "kikapcsolas" hatasat (a felhasznalo csak egy felvillanast latott,
  // ami az utana ujrainduló automatikus mintabol szarmazott). Ez a flag
  // biztositja, hogy manualis "ki" eseten a LED tenyleg stabilan LOW
  // maradjon, amig a felhasznalo explicit vissza nem kapcsol automatara.
